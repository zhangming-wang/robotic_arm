...
def generate_launch_description():
    ...
    # 仿真模式：use_sim:=true 切换到 gz_ros2_control/GazeboSimSystem 插件
    robot_description = {
    ...
    # 启动 Gazebo 仿真底层（RViz 由下面 MoveIt 的 rviz_node 接管）
    arm_bringup = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                arm_description_share, "launch", "arm_description_sim.launch.py"
            )
        ),
        launch_arguments={
            "start_rviz": "false",

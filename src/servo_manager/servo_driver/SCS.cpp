...
int SCS::snycRead(u8 ID[], u8 IDN, u8 MemAddr, u8 *nDat, u8 nLen) {
    syncReadBegin(sizeof(ID), nLen);
    syncReadPacketTx(ID, IDN, MemAddr, nLen);
    int ret = 0;
    for (u8 i = 0; i < IDN; i++) {
        if (syncReadPacketRx(ID[i], nDat + i * nLen)) {
            ret++;
        }
    }
    syncReadEnd();
